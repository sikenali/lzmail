			...
			for job := range jobCh {
				if err := executeJob(&job); err != nil {
					...
					if ScheduledStoreInstance != nil {
						if next, rerr := ScheduledStoreInstance.RecordFailure(job.ID); rerr != nil {
							log.Printf("[send] record failure for job %d failed: %v", job.ID, rerr)
						} else if next.IsZero() {
							log.Printf("[send] scheduled job %d gave up after %d attempts", job.ID, store.MaxSendAttempts)
						} else {
							log.Printf("[send] scheduled job %d retry at %s", job.ID, next.Format(time.RFC3339))
						}
					}
					continue
				}
...
func buildMessage(from, to, cc, subject, bodyText, bodyHTML string, attachments []ComposeAttachment, extraHeaders ...string) []byte {
	...
	// 解析 extraHeaders（键值对交替）
	for i := 0; i+1 < len(extraHeaders); i += 2 {
		headers[extraHeaders[i]] = extraHeaders[i+1]
	}
...
func validateAttachmentPaths(uploadDir string, atts []ComposeAttachment) ([]ComposeAttachment, error) {
	...
	for i, a := range atts {
		...
		// 兼容两种存储：绝对路径（历史数据）或相对文件名（新上传的附件）
		if !filepath.IsAbs(a.Path) {
			a.Path = filepath.Join(root, a.Path)
		}
...
func (h *Handler) persistOutgoing(account *models.Account, req *ComposeRequest, folder string, when time.Time) {
	...
	extra := []string{}
	if req.ReplyTo != "" {
		extra = append(extra, "In-Reply-To", req.ReplyTo)
	}
	if req.References != "" {
		extra = append(extra, "References", req.References)
	}
	msg := buildMessage(account.Email, req.To, req.Cc, req.Subject, req.BodyText, req.BodyHTML, req.Attachments, extra...)
...
func (h *Handler) handleCompose(w http.ResponseWriter, r *http.Request) {
	...
	if req.ReplyTo != "" || req.References != "" {
		headers := []string{}
		if req.ReplyTo != "" {
			headers = append(headers, "In-Reply-To", req.ReplyTo)
		}
		if req.References != "" {
			headers = append(headers, "References", req.References)
		}
		msg = buildMessage(account.Email, req.To, req.Cc, req.Subject, req.BodyText, req.BodyHTML, req.Attachments, headers...)
	}
...
func sendMail(addr string, a smtp.Auth, from string, to []string, msg []byte) error {
	...
	// 设定整体超时，防止 SMTP 服务器无响应时 HTTP 请求无限挂起
