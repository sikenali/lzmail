...
export default function ContactPicker({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (!EMAIL_RE.test(query.trim())) return
    e.preventDefault()
    addManualEmail(query)
  }

  const handleConfirm = () => {
    const contactEmails = [...selected].map(id => {
      const c = allContacts.find(cc => String(cc.id) === id)
      return c?.email || ''
    }).filter(Boolean)
    const emails = [
      ...manualEmails,
      ...contactEmails.filter(e => !manualEmails.includes(e)),
    ]
  ...
  return (
    <div ref={wrapperRef} className="relative flex-1 min-w-0">
      ...
      >
        {/* 已选联系人标签 */}
        {(selectedCount > 0 || manualEmails.length > 0) && (
          <div className="flex flex-wrap gap-1.5 flex-1 min-w-0">
            {manualEmails.map(email => (
              ...
              >
                {email}
                <button
                  onClick={e => { e.stopPropagation(); setManualEmails(prev => prev.filter(x => x !== email)) }}
        ...
        <input
          ...
          placeholder={selectedCount === 0 && manualEmails.length === 0 ? placeholder : ''}
      ...
      {open && (
        ...
        >
          ...
          {/* 手输邮箱提示 */}
          {query.trim().length > 0 && !EMAIL_RE.test(query.trim()) && (
            <div className="px-3 py-2 text-[11px]" style={{ color: 'var(--muted-foreground)' }}>请输入完整邮箱后按回车</div>
          )}
          ...
          {loading ? (
            ...
            EMAIL_RE.test(query.trim()) ? (
              <div
                className="px-3 py-2.5 cursor-pointer transition-colors hover:bg-[rgba(243,237,227,1)]"
                style={{ color: 'var(--foreground)' }}
                onClick={() => addManualEmail(query)}
              >
                添加 <span className="text-[13px] font-medium">{query.trim()}</span>
              </div>
            ) : (
              <div className="px-4 py-8 text-center text-[13px]" style={{ color: 'var(--muted-foreground)' }}>
                未找到匹配的联系人
