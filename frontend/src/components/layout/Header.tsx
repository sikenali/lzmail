'use client'
import { Search, Settings, User, getAccountAvatarBg, Contact } from '@/lib/icons'
...
export function Header({ onCompose }: { onCompose: () => void }) {
  ...
  const showSearch = pathname === '/' || pathname === '/mail' || pathname === '/contacts' || pathname.includes('?id=')
  ...
  return (
      ...
      <div className="flex items-center gap-4">
        <Link href="/contacts" className="w-10 h-10 flex items-center justify-center rounded-lg hover:opacity-90 transition-opacity" style={{ backgroundColor: 'var(--primary)' }}>
          <Contact className="w-5 h-5" style={{ color: '#ffffff' }} />
        </Link>
        <Link href="/settings" className="w-10 h-10 flex items-center justify-center rounded-lg hover:opacity-90 transition-opacity" style={{ backgroundColor: 'var(--primary)' }}>
          <Settings className="w-5 h-5" style={{ color: '#ffffff' }} />
        </Link>

        {/* User avatar - uses account brand_color or accent color */}
        <div
          className="w-10 h-10 rounded-lg flex items-center justify-center cursor-pointer transition-all hover:opacity-90"
        ...
        >
          <svg viewBox="0 0 24 24" style={{ width: '20px', height: '20px' }}>
            <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" fill="#ffffff" />
            <polyline points="22,6 12,13 2,6" fill="none" stroke="var(--foreground)" strokeWidth="2" />
          </svg>
        </div>
      </div>
    </div>
