...
const avatarGradients: Record<string, string> = {
  '#ea4335': 'from-orange-400 to-red-500',
  '#0078d4': 'from-blue-400 to-indigo-500',
  '#12b7f5': 'from-cyan-400 to-sky-500',
  '#e53e3e': 'from-red-400 to-rose-500',
}
...
export function MailItem({ email, brand, onSelect }: { email: Email; brand?: string; onSelect: (id: number) => void }) {
  ...
  return (
    <div onClick={() => onSelect(email.id)}
      className="flex items-center gap-3 px-5 py-3 cursor-pointer transition-colors hover:bg-gray-50 border-b"
      style={{ borderColor: '#f9fafb' }}
    >
      <div className="w-1 h-10 rounded-full shrink-0" style={{ backgroundColor: barColor, borderRadius: '9999px' }} />
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold truncate" style={{ color: '#1e293b' }}>{email.from}</span>
        ...
        <div className="text-sm font-medium truncate mt-0.5" style={{ color: '#334155' }}>{email.subject || '(无主题)'}</div>
        <div className="text-xs truncate mt-0.5" style={{ color: '#94a3b8' }}>{email.body_preview}</div>
      </div>
